...
plt.scatter(X[Y_kmeans==2,0], X[Y_kmeans==2,1], s=100, color='green', label='cluster 1' )
plt.scatter(X[Y_kmeans==3,0], X[Y_kmeans==3,1], s=100, color='orange', label='cluster 1' )
plt.scatter(X[Y_kmeans==4,0], X[Y_kmeans==4,1], s=100, color='purple', label='cluster 1' )
...
plt.legend()
plt.show()
#first valuea in both X and Y are cluster number  
#while the second values are column 0 for x axis and column 1 for y axis
